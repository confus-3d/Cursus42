...
/*   Updated: 2023/09/26 16:28:01 by fde-los-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	flag = 1;
	while (ft_strchr(copytext[fd], '\n') == NULL && flag > 0)
		flag = get_next_read(fd, buffer, &copytext[fd]);
	free(buffer);
	buffer = NULL;
	if (flag == -1 || ft_strlen(copytext[fd]) == 0
		|| get_next_jump(&copytext[fd], &line) == -1
		|| remove_last_line(&copytext[fd], &line) == -1)
		return (NULL);
	return (line);
}
